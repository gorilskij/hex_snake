...
use crate::basic::{HexDim, Dir, Point, CellDim, HexPoint};
...
impl Controller for Mouse {
    ...
    fn get_mesh(&self, gtx: &GameContext, ctx: &mut Context) -> Option<GameResult<Mesh>> {
        let mouse_position: Point = mouse::position(ctx).into();
        ...
        Some(Mesh::new_polygon(ctx, draw_mode, &hexagon, Color::CYAN))
    }
}
