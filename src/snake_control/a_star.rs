...
impl AStar {
    ...
    fn occupied_positions(&self, body: &Body, other_snakes: impl Snakes) -> HashSet<HexPoint> {
        body.segments
            .iter()
            .filter(|seg| !self.passthrough_knowledge.can_pass_through_self(seg))
            .chain(other_snakes.iter().flat_map(|snake| {
    ...
    fn recalculate_path(&mut self, body: &Body, other_snakes: impl Snakes, board_dim: HexDim) {
        ...
        let forbidden_positions = {
            ...
            fp.insert(body.segments[0].pos.translate(-body.dir, 1));
            fp
        };
    ...
    fn next_dir(
        ...
        let going_to_crash = || {
            self.path
                .get(0)
                .map(|dir| {
                    let pos = body.segments[0].pos.translate(*dir, 1);
        ...
        if self.target.is_none()
            ...
            || going_to_crash()
        {
            self.recalculate_target(body.segments[0].pos, apples, gtx.board_dim);
        ...
        if !self.path.is_empty() {
            ...
            Self::least_damage(body.segments[0].pos, body, other_snakes, gtx.board_dim)
        }
    }
